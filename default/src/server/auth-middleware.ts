...
export async function authHandler(req: Request, res: Response) {
  try {
    const baseURL = 'https://syntract.net';
    console.log(JSON.stringify({ event: 'auth.request', method: req.method, path: req.path, baseURL, origin: req.get('origin') }));

    const auth = getAuth(baseURL);
    const webResponse = await auth.handler(toWebRequest(req));

    // Log non-2xx auth responses to help debug issues
    if (webResponse.status >= 400) {
      const body = await webResponse.clone().text();
      console.error(JSON.stringify({ event: 'auth.response.error', status: webResponse.status, path: req.path, body }));
    }
